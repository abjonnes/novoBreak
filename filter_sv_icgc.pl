...
while (<>) {
	...
	if ($len < 1000 and $len > 100 and $e[18] + $e[23] > 4 and $e[13] > 3 and $e[5] > 10) { # small svs
		print if $e[15]>0 and $e[16]/$e[15]>0.1 or ($e[20]>0 and $e[21]/$e[20]>0.1);
	...
	} elsif ($e[25]>=3 and $e[26]>=3 or ($e[25]<=1 and $e[26]<=1 and $e[13]>4 and $e[18]>=2 and $e[23]>=2)) { # this may be a better filter for real data
#	elsif ($e[36] <= 1 and $e[38] <= 1 and $e[23] <= 1 and $e[33] <= 1) {
		if (/INV/) {
			#print if $len < 20000000;
			print;
		} elsif (/TRA/) {
			print if $e[25] > 3 or $e[26] > 3;
		} else {
			print;
		}
	} elsif (/TRA/) {
		print if ($e[5] > 30 and $e[18]+$e[23] > 10);
	}
}
